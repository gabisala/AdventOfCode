...
# Process input data
with open('AdventOfCode/2022/Day_1/input.txt', 'r') as file:
...
for line in lines:
    if line != '\n':
        food.append(int(line.rstrip('\n')))
    else:
        elfs_food.append(food)
        food = []
elfs_food.append(food)

# First puzzle
def carrying_most_calories(elfs_food):
    return max([sum(food_calories) for food_calories in elfs_food])

most_calories = carrying_most_calories(elfs_food)
print(most_calories)

# Second puzzle
def sum_top_3_most_calories(elfs_food):
    all_backpacks = [sum(food_calories) for food_calories in elfs_food]
    sorted_backpacks = sorted(all_backpacks, reverse=True)
    return sum(sorted_backpacks[:3])
